...
// Alocação 1D contígua e alinhada
...
// Transposição para garantir acesso linear na matriz B
...
int main(int argc, char* argv[]) {
    ...
    // Transposição (preparação)
    transpor_matriz(matriz2, matriz2_t, TAMANHO);

    struct timeval start, end;
    gettimeofday(&start, NULL);

    // KERNEL SEQUENCIAL OTIMIZADO
    for (int i = 0; i < TAMANHO; i++) {
        int offset_A = i * TAMANHO;
        for (int j = 0; j < TAMANHO; j++) {
            int offset_B = j * TAMANHO; // B transposta = acesso linear
            int soma = 0;
            // O compilador vetoriza este loop automaticamente com -O3
    ...
    printf("SEQUENCIAL | Matriz: %dx%d | Tempo: %.6f s\n", TAMANHO, TAMANHO, tempo);
